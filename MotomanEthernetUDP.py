...
class UdpPacket_Req(UdpPacket):
    ...
    #     l_str = (
    #         self.identifier.encode('utf-8') +                  # Identifier 4 Byte Fixed to “YERC”
    #         bytes([self.headSize[0]]) +           # Header part size 2Byte Size of header part (fixed to 0x20)
    #         bytes([self.headSize[1]]) +
    #         bytes([self.dataSize[0]]) +           # Data part size 2Byte Size of data part (variable)
    #         bytes([self.dataSize[1]]) +
    #         bytes([self.reserve1]) +              # Reserve 1 1Byte Fixed to “3”
    #         bytes([self.procDiv]) +               # Processing division 1Byte 1: robot control, 2: file control
    #         bytes([self.ACK]) +
    #         bytes([self.reqID]) +                 # Request ID 1Byte Identifying ID for command session
    #         # (increment this ID every time the client side outputs a
    #         # command. In reply to this, server side answers the received
    #         # value.)
    #         bytes([self.blockNo[0]]) +             # Block No. 4Byte Request: 0
    #         bytes([self.blockNo[1]]) +                  # Answer: add 0x8000_0000 to the last packet.
    #         bytes([self.blockNo[2]]) +                  # Data transmission other than above: add 1
    #         bytes([self.blockNo[3]]) +                  # (max: 0x7fff_ffff)
    #         self.reserve2.encode('utf-8') +                    # Reserve 2          8 Byte       Fixed to “99999999”
    #         bytes([self.cmdNo[0]]) +
    #         bytes([self.cmdNo[1]]) +
    #         bytes([self.inst[0]]) +
    #         bytes([self.inst[1]]) +
    #         bytes([self.attr]) +
    #         bytes([self.service]) +
    #         bytes([self.padding[0]]) +
    #         bytes([self.padding[1]])
    #     )

    #     if isinstance(self.data, str):
    #         # data is string
    #         l_str = l_str + self.data.encode('utf-8')
    #     else:
    #         for i in range(self.dataSize[0]):
    #             l_str = l_str + bytes([self.data[i]])
