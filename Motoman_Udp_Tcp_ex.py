...
class rob():
	...
	def __init__(self, PARENT=0, dbg = 0):
		...
		self.IP_ADD 	= '192.168.255.200'					#robot IP
...
#~ -----------------------------------------------------------------------------------------------------------------------------------------
	...
	def mvpath(self, pts=[], inc=0, pls=0, xyz=0, jnt=0, lin=0, ind=0):							#multipoint move
...
#~ -----------------------------------------------------------------------------------------------------------------------------------------
	...
	def udp_rtrq(self):																		#udp read joint torque
		...
		self.sock_udp.sendto(comm,("192.168.255.200",10040))
		...
		if len(data) > 32:		
			...
			reqdat = data[32:]
			for i in range(0,len(reqdat),4): nib.append(reqdat[i:i+4])
	...
	def udp_rpos(self, p=0):																#udp read position 		
		...
		if len(data) > 32:		
			...
			for i in range(0,len(reqdat),4): nib.append(reqdat[i:i+4]) 			#separate data words and extract requested data
